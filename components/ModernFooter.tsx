"use client";
import Link from "next/link";
import React from "react";

const navigation = {
  product: [
    { name: "How It Works", href: "#how-it-works" },
    { name: "Features", href: "#features" },
    { name: "Documentation", href: "https://docs.yukiprotocol.com" },
    { name: "Roadmap", href: "#" },
  ],
  support: [
    { name: "FAQ", href: "#" },
    { name: "Community", href: "https://discord.gg/ZuS6Mj4r8j" },
    { name: "Security", href: "#" },
    { name: "Contact", href: "mailto:info@yukiprotocol.com" },
  ],
  legal: [
    { name: "Privacy Policy", href: "#" },
    { name: "Terms of Use", href: "#" },
    { name: "Disclaimer", href: "#" },
    { name: "Risk Disclosure", href: "#" },
  ],
  social: [
    {
      name: "Discord",
      href: "https://discord.gg/ZuS6Mj4r8j",
      icon: (props: React.SVGProps<SVGSVGElement>) => (
        <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
          <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
        </svg>
      ),
    },
    {
      name: "Twitter",
      href: "https://x.com/yukiprotocol",
      icon: (props: React.SVGProps<SVGSVGElement>) => (
        <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      ),
    },
    {
      name: "GitHub",
      href: "https://github.com/yukiprotocol",
      icon: (props: React.SVGProps<SVGSVGElement>) => (
        <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
        </svg>
      ),
    },
  ],
};

export default function ModernFooter() {
  return (
    <footer className="border-t border-cfd0ce/20 pt-16 pb-8 relative overflow-hidden bg-fdfffc">
      <div className="absolute top-0 right-0 w-1/3 h-1/3 bg-0f52fb/5 rounded-bl-full blur-3xl"></div>
      <div className="absolute bottom-0 left-0 w-1/4 h-1/4 bg-0f52fb/5 rounded-tr-full blur-3xl"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          <div className="md:col-span-1">
            <Link href="/" className="flex items-center mb-6">
              <span className="font-gloock text-2xl text-0f52fb">Yuki</span>
            </Link>
            <p className="text-303130/70 text-sm mb-6">
              A simplified approach to DeFi participation with transparency and
              efficiency at its core.
            </p>
            <div className="flex space-x-4">
              {navigation.social.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="text-303130/60 hover:text-0f52fb transition-colors duration-300"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <span className="sr-only">{item.name}</span>
                  <item.icon className="h-6 w-6" aria-hidden="true" />
                </Link>
              ))}
            </div>
          </div>

          <div>
            <h3 className="text-303130 text-sm font-medium mb-4">Product</h3>
            <ul className="space-y-3">
              {navigation.product.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-303130/70 text-sm hover:text-0f52fb transition-colors duration-300"
                  >
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-303130 text-sm font-medium mb-4">Support</h3>
            <ul className="space-y-3">
              {navigation.support.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-303130/70 text-sm hover:text-0f52fb transition-colors duration-300"
                    target={item.href.startsWith("http") ? "_blank" : undefined}
                    rel={
                      item.href.startsWith("http")
                        ? "noopener noreferrer"
                        : undefined
                    }
                  >
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h3 className="text-303130 text-sm font-medium mb-4">Legal</h3>
            <ul className="space-y-3">
              {navigation.legal.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className="text-303130/70 text-sm hover:text-0f52fb transition-colors duration-300"
                  >
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </div>

        <div className="border-t border-cfd0ce/20 pt-8">
          <div className="bg-cfd0ce/10 p-4 text-xs text-303130/80 leading-relaxed rounded-md">
            <p>
              <strong>Risk Disclosure:</strong> Digital assets involve
              significant risks, including price volatility, technical
              vulnerabilities, and regulatory uncertainty. The Yuki Protocol is
              under development. Any information presented on this website is
              for informational purposes only and does not constitute investment
              advice or an offer to sell securities. Past performance is not
              indicative of future results. Before interacting with any digital
              asset protocol, users should conduct their own research and
              consult appropriate professionals.
            </p>
          </div>

          <div className="mt-8 flex flex-col sm:flex-row justify-between items-center">
            <p className="text-xs text-303130/60">
              &copy; {new Date().getFullYear()} Yuki Protocol. All rights
              reserved.
            </p>
            <p className="mt-2 sm:mt-0 text-xs text-303130/60">
              Not available to residents of certain jurisdictions
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
}
